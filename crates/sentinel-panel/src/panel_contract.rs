...
pub(crate) fn active_blocks_private_dataset() -> PanelDataset {
    PanelDataset {
        ...
        columns: &["id", "blocked_at", "node_id AS node_name", "ip", "(SELECT network_prefix FROM probe_sources WHERE probe_sources.source_ip = active_blocks.ip AND network_prefix IS NOT NULL AND network_prefix <> '' AND LOWER(network_prefix) <> 'unknown' ORDER BY probe_sources.last_seen DESC LIMIT 1) AS network_prefix", "(SELECT country FROM probe_sources WHERE probe_sources.source_ip = active_blocks.ip AND country IS NOT NULL AND country <> '' AND LOWER(country) <> 'unknown' ORDER BY probe_sources.last_seen DESC LIMIT 1) AS country", "(SELECT asn FROM probe_sources WHERE probe_sources.source_ip = active_blocks.ip AND asn IS NOT NULL AND asn <> '' AND LOWER(asn) <> 'unknown' ORDER BY probe_sources.last_seen DESC LIMIT 1) AS asn", "(SELECT organization FROM probe_sources WHERE probe_sources.source_ip = active_blocks.ip AND organization IS NOT NULL AND organization <> '' AND LOWER(organization) <> 'unknown' ORDER BY probe_sources.last_seen DESC LIMIT 1) AS organization", "(SELECT categories_json FROM probe_sources WHERE probe_sources.source_ip = active_blocks.ip AND categories_json IS NOT NULL AND categories_json <> '' AND categories_json <> '[]' ORDER BY probe_sources.last_seen DESC LIMIT 1) AS categories_json", "rule_id", "backend", "reason", "expires_at"],
    }
}
