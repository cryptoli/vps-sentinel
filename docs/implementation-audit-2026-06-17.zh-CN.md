...
- Scoring model：进程、deleted executable、持久化命令使用风险评分，不靠单一字符串；
...
| 关键文件完整性 | 对配置路径做哈希，基线对比后检测关键系统文件漂移。 | 能发现持久化和权限相关文件变化；软件包更新也可能触发，所以建议关联变更记录。 | `/etc/passwd` 漂移正例，非关键应用文件漂移反例。 |
| WebShell 文件 | 文件采集限制大小，扫描 Web 路径内容 marker 和脚本/可执行属性。 | WebShell marker 可能命中合法代码，因此消息建议先确认再隔离。 | WebShell marker 正例、干净 Web 文件反例、Web 目录可执行文件正例、非 Web 可执行文件反例。 |
| 用户与权限 | 基线对比 `/etc/passwd`，检测新增用户、UID 0 用户、用户属性变化。 | UID 0 非 root 是强信号；新增用户可能是正常运维。 | 新用户、UID 0、用户修改正例；普通快照和正常 UID 反例。 |
| 持久化文件 | 采集 cron、systemd、shell profile、ld.so.preload，做基线漂移检测。 | 能发现新增/修改启动项；合法软件安装也可能触发，需要基线管理。 | systemd/cron 漂移、ld preload 漂移正例；当前快照和普通 systemd 漂移反例。 |
...
| 脚本和部署 | install/update/reload/stop 脚本，systemd unit，配置校验后重载。 | 更新不默认刷新基线，避免把未审计漂移变成可信状态。 | shell `bash -n`、VPS update、config validate、scan --no-notify。 |
...
本轮发现并修复了一个应配置化的业务阈值：`WEB-002` 的 20 次错误阈值。现在改为：
...
- 合法软件包更新可能触发文件/持久化基线漂移，需要确认后刷新基线。
- WebShell marker 可能命中合法管理脚本，因此消息建议确认后再隔离。
- 已改名、无明显网络执行桥、无临时路径的恶意进程可能漏报，需要更多行为信号补充。
