...
默认公开菜单就是 `overview,probe_sources,nodes`。如果希望所有页面都必须输入 token，将其设为空：`PANEL_PUBLIC_PAGES=`。公开层只返回聚合趋势、风险数量、等级分布、节点名称、在线状态、非敏感节点指标和脱敏后的封禁黑名单摘要，不返回原始证据、命令行、文件路径、服务器 IP、密钥、日志或敏感配置。
