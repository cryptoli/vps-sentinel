...
访问 `PANEL_ADMIN_PATH` 配置的路径（默认 `/admin`）会显示管理端 token 输入页。输入 `PANEL_ADMIN_TOKEN` 后可查看全部页面；`PANEL_OPERATOR_TOKEN` 只查看运维层数据。
...
浏览器 API 需要 token。输入 `PANEL_OPERATOR_TOKEN` 或 `PANEL_ADMIN_TOKEN`。如果希望无需 token 查看指定公开菜单，可以显式设置：

```bash
PANEL_PUBLIC_PAGES=overview,probe_sources,nodes
```

默认公开菜单就是 `overview,probe_sources,nodes`。如果希望所有页面都必须输入 token，将其设为空：`PANEL_PUBLIC_PAGES=`。公开层只返回聚合趋势、风险数量、等级分布、节点名称、在线状态、非敏感节点指标，以及已确认封禁的外部来源 IP、国家、ASN 和组织；不返回原始证据、命令行、文件路径、服务器 IP、密钥、日志或敏感配置。
