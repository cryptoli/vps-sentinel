...
| 组件 | 职责 |
| --- | --- |
| Agent 面板客户端 | 生成有界遥测信封，移除敏感主机身份和原始证据，补充安全展示字段，使用 HMAC 签名，并把失败上报写入本地队列。 |
| 接收端 | 校验签名、nonce、时间戳和 payload 大小；合并 agent 元数据、可信代理元数据或可选本地 GeoIP 数据库中的非敏感节点位置；入库前保存脱敏后的行数据。 |
...
面板 API 按数据集和列白名单返回数据。公开页面只暴露聚合信息或明确可公开的外部攻击源信息。私有页面可以查看复核和处置细节。节点 ID、主机 ID、服务器公网 IP、原始路径、命令行、密钥和复核签名不会出现在浏览器列表接口中；脱敏后的非 IP 主机名只在私有节点数据集中提供。
