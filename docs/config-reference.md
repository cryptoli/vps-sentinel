...
- `[agent]`: host ID, hostname, human-readable `display_name`, scan interval, data directory, log level. Notification subjects use `display_name`, then `hostname`, then `host_id`, then `local-host`.
...
- `[process]`: process scan thresholds and suspicious directories. `deleted_executable_min_score` controls when `PROC-002` is emitted; deleted executable state must combine with suspicious traits such as temporary paths, memfd or anonymous backing, hidden non-standard paths, network execution bridges, sustained high CPU, or known bad tool identity. `PROC-003` profiles `/proc/<pid>/cmdline` argv and requires high-confidence network command-execution bridge behavior, not traffic-forwarding tool names alone. `PROC-004` is a known-tool indicator rule that matches miner/scanner names against process identity fields such as executable path, process name, and structured `argv[0]`; legacy events without structured identity fall back to command token basename matching. `high_cpu_threshold_percent` and `high_cpu_duration_seconds` use procfs lifetime CPU metrics as supporting behavior evidence. `behavior_min_score` and `suspicious_socket_fd_threshold` control `PROC-005`, which combines weak signals such as kernel-thread masquerading, web-root execution, hidden executable names, suspicious cwd, socket-FD activity, sustained high CPU, and effective-root context.
...
- `[network]`: listening port policy. `expected_public_ports` suppresses ordinary exposed-service noise but still allows process-risk and baseline-owner checks, `high_risk_public_ports` controls ports that are risky when public, and `alert_on_new_listening_port` reports ordinary new TCP/TCP6 listeners only when they appear relative to the stored baseline. Loopback, RFC1918 IPv4, IPv6 ULA, and link-local listeners are not treated as public exposure; `0.0.0.0`, `::`, and specific routable public addresses are. Generic UDP high ports are treated as dynamic traffic unless they match a high-risk service port or suspicious listener process. `public_listen_allowlist` is kept as a legacy alias for expected public ports; use `[allowlist].listening_ports` to suppress all network findings for a port.
...
`file_integrity.max_depth`, `file_integrity.webshell_min_score`, `process.deleted_executable_min_score`, `process.behavior_min_score`, `process.high_cpu_threshold_percent`, `process.high_cpu_duration_seconds`, `process.suspicious_socket_fd_threshold`, `package_manager.recent_activity_window_seconds`, and `package_manager.max_log_tail_bytes` are validated as positive values so a misconfigured zero cannot accidentally turn broad matching into alert spam. Enabled notification channels are also validated for required delivery settings before the daemon accepts the configuration.
