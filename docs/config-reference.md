...
`file_integrity.webshell_min_score`, `process.deleted_executable_min_score`, `process.behavior_min_score`, `process.high_cpu_threshold_percent`, `process.high_cpu_duration_seconds`, `process.suspicious_socket_fd_threshold`, `package_manager.recent_activity_window_seconds`, and `package_manager.max_log_tail_bytes` are validated as positive values so a misconfigured zero cannot accidentally turn broad matching into alert spam.
