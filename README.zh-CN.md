...
| 多 VPS 面板 | 将有签名、有大小上限的遥测推送到 Rust 自建面板或 Cloudflare Worker/D1 接收端；节点 ID、主机名、原始证据和通用网络字段会脱敏，管理层专用的主动封禁和外部探查黑名单数据集会保留公网来源 IP、封禁原因和基础网段信息，便于复核处置。面板同时展示 CPU、内存、负载、在线时长、流量、网络速率和 agent RSS 等轻量探针指标。自建 UI 使用 WebSocket 变更事件，浏览器 API 按公开、运维、管理三层授权。 |
...
常用命令是 `vs panel push`、`vs panel flush` 和 `vs panel outbox`。`panel.ip_intel_paths` CSV 会在本地按最长 CIDR 前缀匹配，`panel.ip_intel_remote_enabled` 会通过可配置 WHOIS 端点对公网外部探查 IP 做补全；两者都会为黑名单和生效封禁补充国家、ASN 和组织信息，同时不上传节点私有身份信息。自建和 Cloudflare 面板都支持用 `PANEL_ADMIN_PATH` 配置 token 管理入口路径，并用 `PANEL_THEMES` 注册第三方 CSS 主题包。Rust 面板服务、Cloudflare Worker/D1、MySQL/PostgreSQL 注意事项和第三方主题/页面开发见 [docs/panel.md](docs/panel.md)；从零部署 Cloudflare 面板和 VPS 自建面板的详细教程见 [docs/panel-deployment.zh-CN.md](docs/panel-deployment.zh-CN.md)。
