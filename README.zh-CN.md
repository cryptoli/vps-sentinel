...
| 进程检查 | 读取 procfs argv 和可执行文件元数据，识别临时目录执行、已删除可执行文件仍在运行、网络命令执行桥接，以及按命令 token 或可执行文件 basename 边界匹配的已知挖矿/扫描器工具名。 |
...
已知挖矿/扫描器检测会更克制：`PROC-004` 只在命令 token 或可执行文件 basename 边界命中 `xmrig`、`masscan`、`zmap` 等已知工具名，并兼容 `.exe` 后缀，不会因为普通长名称里包含这些子串就告警。
