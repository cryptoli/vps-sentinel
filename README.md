...
| File and persistence drift | Builds a local SQLite baseline, diffs later snapshots, coalesces related file/persistence findings for the same path, and adds a semantic drift score with review tier and package-manager context. | Finds real drift while reducing confusion during legitimate package updates; sensitive changes such as SSH keys, UID 0, and preload drift remain high-value until reviewed. |
...
| Incidents and timeline | Groups related findings by IP, path, process, category, and time window, stores a bounded incident index, and emits scan-window timeline findings for multi-phase chains. | Turns isolated findings into a readable attack-chain view while keeping raw findings available. |
...
Auditd reads configured audit logs when available. Audit EXECVE events are converted into the same RawEvent model and can identify network-to-shell execution bridges or non-interactive sudo/su/pkexec shell execution using the shared command-profile logic. The eBPF bridge expects JSONL events from files or a configured command, which lets operators integrate their own BPF tooling without making kernel probes a hard dependency. Sigma-like rules are TOML files containing structured event field conditions; validate them with `vs rules validate-external <path...>` before deployment. YARA support calls the configured `yara` binary only when rule paths and scan roots are configured, so the default engine is active but does no YARA work until inputs exist.
