...
| Baseline drift | Creates local baselines for users, SSH keys, critical files, persistence entries, and listeners; compares future scans against the stored baseline, scores drift by sensitivity, exposure, change magnitude, and maintenance context, then exposes approval keys for reviewed refreshes. |
...
| Incident correlation | Correlates findings by source IP, path, process, category, and time window into local incidents and scan-window attack timelines. |
...
| Fleet panel | Pushes signed, bounded telemetry to a self-hosted Rust panel or a Cloudflare Worker/D1 receiver; node IDs, hostnames, raw evidence, and general network fields are redacted, while confirmed external blocklist IPs can be shown with country, ASN, and organization. The panel also shows lightweight node probe metrics such as CPU, memory, load, uptime, traffic totals, and network rates. The self-hosted UI uses WebSocket change events and browser APIs are scoped into public, operator, and admin roles. |
...
| Active response | Handles high-confidence public-source web probes and SSH brute-force sources through `observe`, `balanced`, or `strict` strategies; evidence layers adjust TTL and permanent-escalation thresholds while firewall writes still use nftables or iptables with public-IP safety checks. Enabled by default for new installs. |
...
File and persistence baseline drift is not suppressed just because package-manager activity exists. Instead, the agent collects recent apt/dpkg/yum/dnf/pacman/apk log activity and combines it with change magnitude, executable-state changes, public exposure, service-owner drift, UID changes, and sensitive-object context to produce a bounded drift score and review action. This keeps real drift visible while making legitimate package updates easier to confirm before refreshing the baseline.
...
Every finding is enriched with a unified 0-100 risk score derived from severity, detector confidence, rule-specific scores, active-response context, evidence strength, and optional threat-intel matches. Evidence scoring is stateless and bounded to the current finding: strong source, volume, process, network, GPU, anti-forensics, and response signals raise confidence, while maintenance/package-manager context can downgrade noisy drift. Scan-window timelines are generated after active-response annotation by grouping related findings on source IP, path, and process identity, so multi-phase chains are visible without storing a large resident graph.

Attack fingerprints are generated after finding coalescing and risk scoring. The fingerprint engine extracts stable features from Web probes, SSH brute-force dictionaries, process behavior, and persistence/file-change findings; normalizes volatile values such as numeric IDs, path variants, and encoded URL forms; derives an exact BLAKE3 hash plus a 64-bit SimHash for near-match clustering; and stores bounded observations in SQLite. Source IPs are not part of the exact fingerprint, so a scanner that rotates IPs can still be grouped by method. `vs fingerprints explain <id>` summarizes the cluster tier, strongest signals, limitations, top features, recent observations, and recommended operator action. If `privacy.mask_ip = true`, stored source-IP values are replaced with stable hashes. A repeated high-score fingerprint can add an active-response action hint to the current finding, but firewall writes still go through the normal public-IP, allowlist, trusted-proxy, response-policy, and max-block safeguards.
...
The agent can push signed telemetry to a central panel without exposing SSH or opening inbound ports on monitored VPS nodes. Self-hosted deployments use the Rust binary `vps-sentinel-panel` with SQLite, PostgreSQL, or MySQL. Cloudflare deployments can use the Worker/D1 receiver in `panel/cloudflare` and serve the static UI from `panel/web`. Browser access is separated into public, operator, and admin roles; the self-hosted panel uses WebSocket change events instead of periodic list polling. The panel doubles as a lightweight node probe by showing non-sensitive CPU, memory, load, uptime, traffic, network-rate, and agent-RSS metrics. Local detection and firewall response still use source IPs. Remote payloads remove node IDs, hostnames, and raw evidence details. Confirmed external probe-source blocklists can show attacker IP, country, ASN, and organization publicly; active-block backend details and raw evidence remain admin-scoped.
...
Useful commands are `vs panel push`, `vs panel flush`, and `vs panel outbox`. `panel.ip_intel_paths` CSV files are matched locally by longest CIDR prefix, and `panel.ip_intel_remote_enabled` can enrich public external probe IPs through a configurable WHOIS endpoint. Both feed probe-source and active-block attribution with country, ASN, and organization while keeping private node identity out of panel telemetry. Optional `panel.location` fields are display-only node metadata for the dashboard and should not contain IPs or private hostnames. Self-hosted and Cloudflare panels support `PANEL_ADMIN_PATH` for the token-protected management entry and `PANEL_THEMES` for third-party CSS theme packages. See [docs/panel.md](docs/panel.md) for the Rust panel service, Cloudflare Worker/D1 setup, MySQL/PostgreSQL notes, and third-party theme/page development; see [docs/panel-deployment.zh-CN.md](docs/panel-deployment.zh-CN.md) for the detailed Cloudflare and VPS deployment tutorial.
...
| File and persistence drift | Builds a local SQLite baseline, diffs later snapshots, coalesces related file/persistence findings for the same path, records semantic profiles for SSH keys, systemd units, cron, and sudoers files, and adds a semantic drift score with review tier and package-manager context. | Finds real drift while reducing confusion during legitimate package updates; sensitive changes such as SSH keys, UID 0, preload, risky startup commands, and sudo privilege changes remain high-value until reviewed. |
...
| Active response | Evaluates current findings after scan-level coalescing/deduplication and before persisted notification deduplication; only public IPs outside `[allowlist].ips` can be blocked, and unresolved trusted proxy/CDN sources are never block candidates. Web/SSH/fingerprint response layers adjust temporary TTLs and permanent-escalation thresholds according to evidence strength. | Lets operators turn noisy, obvious scanners into firewall drops without making every alert destructive or blocking CDN edges by mistake. |
...
| Attack fingerprints | Extracts normalized feature sets from Web/SSH/process/persistence findings, builds exact hashes and SimHash similarity keys, stores bounded observations, writes fingerprint evidence back to findings, and explains cluster signals on demand. | Groups rotating-IP attacks by method, exposes `vs fingerprints` timelines, explanations, and verdicts, and can make repeated confirmed methods eligible for active response without relying on IP blacklists. |
...
| `vps-sentinel baseline diff --json --config <path>` | Compare current local state against the stored baseline and print reviewable drift approval keys with risk score, tier, review action, and reasons. |
...
| `vps-sentinel blocks why <ip> --config <path>` | Explain why an IP is blocked: backend, TTL/permanent state, firewall verification, source rule, finding ID, reason, and stored finding evidence. Add `--json` for automation. |
...
| `vps-sentinel fingerprints explain <fingerprint_id> --config <path>` | Explain why a fingerprint cluster matters: risk tier, strongest signals, limitations, top normalized features, recent observations, and suggested operator action. |
...
Recent package-manager activity is attached as evidence to file and persistence drift findings. It is not an allowlist and does not refresh the baseline automatically. `vs baseline diff` shows each drift item's score, tier, review action, and reasons so you can confirm routine changes against package logs while treating sensitive or exposed drift as investigation work first.
...
- `expected_public_ports` suppresses generic public-listener noise for intentionally exposed services such as SSH, HTTP, HTTPS, or a business-required Redis endpoint.
...
- `public_listen_allowlist` is treated as a legacy alias for expected public ports. Use `[allowlist].listening_ports` only when you intentionally want to suppress all network findings for a port, including suspicious owner checks.
...
`deleted_executable_min_score` controls when `PROC-002` is emitted. Deleted executable state is scored with path, process identity, and command-behavior traits; a standard system binary left running after a package upgrade is not enough by itself. `behavior_min_score` controls `PROC-005`, which combines weak process signals such as kernel-thread masquerading, web-root execution, hidden executable names, suspicious cwd, socket-FD activity, sustained high CPU, procfs start-time drift for the same process identity, package ownership, systemd `ExecStart` mismatch, bounded outbound connection fanout, and effective-root context. Start-time drift and outbound fanout only strengthen already suspicious process identity or location evidence; a normal restart or busy service does not alert by itself. `high_cpu_threshold_percent` and `high_cpu_duration_seconds` define sustained high CPU using procfs lifetime CPU time and process age; high CPU is a supporting signal, not an alert condition by itself. `suspicious_socket_fd_threshold` defines when socket ownership becomes a stronger behavior signal. `public_outbound_fanout_threshold` defines when public outbound connections become fanout evidence, and `outbound_remote_addr_sample_size` bounds the address sample kept in evidence. `known_bad_tool_names` controls the `PROC-004` known miner/scanner indicator list. Values are matched against process identity fields such as `exe_path`, `executable`, process name, and structured `argv[0]`, with `.exe` suffixes accepted. Legacy events without structured identity fall back to command token basename matching. When several process rules match the same PID, the scanner keeps one highest-value finding and merges the process signals, risk reasons, impact, and recommendations.
...
`PROC-006` is only available when the service can run `nvidia-smi` or `rocm-smi` and see the host GPU compute process table. GPU memory alone is treated as normal workload context. NVIDIA hosts also contribute device utilization and power-draw evidence when available; high utilization or high power draw is still only a supporting signal. Alerts require combined evidence such as a configured GPU miner identity, temporary/deleted/anonymous executable, configured mining-pool port, network execution bridge, high utilization plus miner-like outbound behavior, or hidden GPU executable with public outbound connections. If you run vps-sentinel inside a container, it must have host PID/procfs and GPU runtime visibility to inspect host GPU miners accurately.
...
Active response is enabled by default for new installs. Existing configs are not overwritten during upgrade, so a host that explicitly has `active_response.enabled = false` stays disabled until the operator changes it. `strategy = "observe"` records block candidates without writing firewall rules, `balanced` is the default policy, and `strict` requires stronger evidence for rejected Web probes and SSH brute-force sources. The scanner applies active response after scan-level coalescing/deduplication but before persisted notification deduplication, so an escalating source can still be blocked even when repeated notifications are suppressed. SSH blocking covers `SSH-003` brute-force findings and `SSH-007` brute-force-followed-by-success findings, and defaults to 6 failed logins in the scan window. Web blocking covers successful sensitive responses, high-confidence RCE-style exploit probes, repeated lower-confidence exploit probes, multi-family scan aggregation, and high-volume error bursts, but Web findings marked `proxy_source_unresolved` are never block candidates. Response layers annotate strong signals such as confirmed Web exposure, brute-force followed by success, repeated fingerprint matches, and operator-marked malicious fingerprints; layers can lengthen temporary TTLs and lower permanent-escalation thresholds, while explicit `[response_policy]` settings still override TTL and permanent thresholds. Quiet hours and notification limiters do not prevent blocking. The backend uses nftables when available and falls back to iptables/ip6tables. Current active response writes source-IP blocks only; `cleanup_legacy_port_guards = true` removes old vps-sentinel port-level guard tables/rules that could block business ports after upgrading from earlier builds. Normal blocks are temporary: nftables uses set timeouts and vps-sentinel also stores block state in SQLite so expired entries can be removed on later scans. If the same public source IP repeatedly becomes a block candidate at least `permanent_block_threshold` times within `permanent_block_window_seconds`, it is escalated to a permanent firewall block with no expiry. Permanent escalation still respects `[allowlist].ips`, trusted proxy attribution safety, `strategy = "observe"`, and `max_blocks_per_scan`, and operators can remove it with `vs blocks unblock <ip>` or `vs blocks unblock-all --yes`. Only public routable source IPs are eligible. When one scan creates at most `notification_detail_limit` new blocks, alerts include the blocked IP and reason; larger block bursts produce one summary alert and operators can inspect details with `vs blocks list --no-verify`.
